...
const DemoNotesStep = ({ onAnimationComplete }: { onAnimationComplete: () => void }) => {
    ...
    return (
        <div className="text-center">
            ...
            <h2 className="text-2xl font-bold text-text-primary mb-3">Rich Notes & Backgrounds</h2>
            <p className="text-text-secondary mb-6">Create beautiful notes with rich text formatting and custom backgrounds to set the mood.</p>
...
const DemoCalendarStep = ({ onAnimationComplete }: { onAnimationComplete: () => void }) => {
    ...
    return (
        <div className="text-center">
            ...
            <h2 className="text-2xl font-bold text-text-primary mb-3">Intuitive Calendar</h2>
            <p className="text-text-secondary mb-6">Click a day to see its agenda, or click-and-drag across multiple days to quickly create a multi-day event.</p>
...
const DemoCustomizeStep = ({ onAnimationComplete }: { onAnimationComplete: () => void }) => {
    ...
    return (
        <div className="text-center">
            ...
            <h2 className="text-2xl font-bold text-text-primary mb-3">Total Customization</h2>
            <p className="text-text-secondary mb-6">Make Nexus yours. Change themes, accent colors, navigation layouts, and much more in the Settings menu.</p>
...
const PermissionsStep = ({ onNextStep }: { onNextStep: () => void }) => {
    ...
    return (
        <div className="text-center">
            ...
            <button 
                onClick={onNextStep}
                className="w-full max-w-xs mx-auto px-4 py-3 rounded-md bg-accent text-white font-semibold hover:bg-accent-hover transition-colors"
            >
...
const FinishStep = ({ onFinish }: { onFinish: () => void }) => {
    ...
    return (
        <div className="text-center">
            ...
            <button onClick={onFinish} className="w-full max-w-xs mx-auto px-4 py-3 rounded-md bg-accent text-white font-semibold hover:bg-accent-hover transition-colors shadow-lg animate-fade-in-up">
...
const OnboardingModal: React.FC<OnboardingModalProps> = ({ isOpen, onFinish, isMobile }) => {
    ...
    const stepComponents = useMemo(() => [
        ...
        <PermissionsStep onNextStep={handleNextStep} />,
        <FinishStep onFinish={onFinish} />
    ], [handleAnimationDone, onFinish, handleNextStep]);
    ...
    const showFooter = step < totalSteps - 2; // Don't show footer on Permissions or Finish step
            ...
            <div className={modalClasses}>
                ...
                <div className="p-8 space-y-4">
                     ...
                     <div className="flex justify-between items-center h-10">
                         ...
                         {showFooter && (
                             <button onClick={handleNextStep} disabled={!isNextEnabled} className={`px-4 py-2 rounded-md bg-accent text-white font-semibold hover:bg-accent-hover transition-colors disabled:bg-border-color disabled:cursor-not-allowed ${step > 0 ? '' : 'ml-auto'}`}>
                                 {t('onboarding.next')}
                             </button>
                         )}
                     </div>
